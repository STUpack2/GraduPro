...
import { 
  ...
  ArrowLeft, RefreshCw, Info, Search, Heart, HeartOff, Dumbbell, ClipboardList, Plus, Activity, BarChart3, Sparkles
} from 'lucide-react';
...
import { useAIHealth } from '@/features/ai-coach/hooks/useAIHealth';
...
const AI_EXERCISES_DATA: Exercise[] = [
  { id: "barbell_biceps_curl", name: "Barbell Biceps Curl", category: "Arms", primaryMuscles: ["biceps"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "bench_press", name: "Bench Press", category: "Chest", primaryMuscles: ["chest"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "chest_fly_machine", name: "Chest Fly Machine", category: "Chest", primaryMuscles: ["chest"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Machine" },
  { id: "deadlift", name: "Deadlift", category: "Back", primaryMuscles: ["lower back", "hamstrings"], level: "Advanced", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "decline_bench_press", name: "Decline Bench Press", category: "Chest", primaryMuscles: ["chest"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "hammer_curl", name: "Hammer Curl", category: "Arms", primaryMuscles: ["biceps", "forearms"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Dumbbell" },
  { id: "hip_thrust", name: "Hip Thrust", category: "Legs", primaryMuscles: ["glutes"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "incline_bench_press", name: "Incline Bench Press", category: "Chest", primaryMuscles: ["chest"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "lat_pulldown", name: "Lat Pulldown", category: "Back", primaryMuscles: ["lats"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Machine" },
  { id: "lateral_raise", name: "Lateral Raise", category: "Shoulders", primaryMuscles: ["shoulders"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Dumbbell" },
  { id: "leg_extension", name: "Leg Extension", category: "Legs", primaryMuscles: ["quadriceps"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Machine" },
  { id: "leg_raises", name: "Leg Raises", category: "Core", primaryMuscles: ["abdominals"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Bodyweight" },
  { id: "plank", name: "Plank", category: "Core", primaryMuscles: ["abdominals"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Bodyweight" },
  { id: "pull_up", name: "Pull Up", category: "Back", primaryMuscles: ["lats"], level: "Advanced", secondaryMuscles: [], instructions: [], images: [], equipment: "Bodyweight" },
  { id: "push_up", name: "Push Up", category: "Chest", primaryMuscles: ["chest"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Bodyweight" },
  { id: "romanian_deadlift", name: "Romanian Deadlift", category: "Back", primaryMuscles: ["hamstrings"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "russian_twist", name: "Russian Twist", category: "Core", primaryMuscles: ["abdominals"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Bodyweight" },
  { id: "shoulder_press", name: "Shoulder Press", category: "Shoulders", primaryMuscles: ["shoulders"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Dumbbell" },
  { id: "squat", name: "Squat", category: "Legs", primaryMuscles: ["quadriceps", "glutes"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "t_bar_row", name: "T-Bar Row", category: "Back", primaryMuscles: ["middle back"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Barbell" },
  { id: "tricep_dips", name: "Tricep Dips", category: "Arms", primaryMuscles: ["triceps"], level: "Intermediate", secondaryMuscles: [], instructions: [], images: [], equipment: "Bodyweight" },
  { id: "tricep_pushdown", name: "Tricep Pushdown", category: "Arms", primaryMuscles: ["triceps"], level: "Beginner", secondaryMuscles: [], instructions: [], images: [], equipment: "Cable" },
];
...
const Workouts = () => {
  ...
  const supportedAiExerciseIds = status.state === 'healthy' && status.data.supported_exercises.length > 0 
    ? status.data.supported_exercises 
    : AI_EXERCISES_DATA.map(ex => ex.id); // Fallback to all 22 if offline
  ...
  const filterExercises = (muscleGroup: MuscleGroup) => {
    ...
    const sourceList = trackingMode === 'ai' 
      ? AI_EXERCISES_DATA.filter(ex => supportedAiExerciseIds.includes(ex.id))
      : allExercises;

    const filtered = sourceList.filter(exercise =>
  ...
  const getFilteredExercises = () => {
    const isAi = trackingMode === 'ai';
    const sourceList = isAi 
      ? AI_EXERCISES_DATA.filter(ex => supportedAiExerciseIds.includes(ex.id))
      : allExercises;

    if (!selectedMuscle) {
      const matchingExercises = sourceList.filter(exercise =>
      ...
      // In AI mode, count against ALL muscle groups so Chest/Back/Legs appear.
      // In manual mode, only count against groups from index 3 onward (force types are 0-2).
      const countableGroups = isAi
        ? muscleGroups.filter(g => g.id !== 'favorites')
        : muscleGroups.slice(3, -1);
      ...
      // In AI tracking, ONLY show categories that actually have supported exercises
      if (isAi) {
        return filteredGroups.filter(group => exercisesByMuscle.has(group.id));
      }
  ...
  useEffect(() => {
    ...
    if (!selectedMuscle && searchQuery.trim() === '') {
      setFilteredAllExercises(sourceList);
      return;
    }
    ...
    const results = sourceList.filter(exercise => {
  ...
  }, [searchQuery, allExercises, selectedMuscle, trackingMode, supportedAiExerciseIds]);
    ...
    <div className="container mx-auto space-y-8 p-6 pb-24 max-w-[1920px]">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex justify-center w-full mt-2 mb-2">
          <div className="flex p-1 bg-gray-100 rounded-xl w-full max-w-[400px]">
            <button
      ...
      {trackingMode === 'ai' && activeAiExercise ? (
        <div className="flex flex-col -mx-6 h-[calc(100vh-240px)]">
          <div className="px-6 mb-4 space-y-3">
            <Button 
              variant="outline" 
              onClick={() => setActiveAiExercise(null)} 
              className="w-full justify-start border-black/10 dark:border-white/10 bg-white dark:bg-bg-card hover:bg-gray-100 dark:hover:bg-bg-secondary text-gray-900 dark:text-white"
            >
              <ArrowLeft className="w-4 h-4 mr-2" /> {t("workouts.back_to_library", { defaultValue: "Back to Library" })}
            </Button>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white pl-1">
              {AI_EXERCISES_DATA.find(e => e.id === activeAiExercise)?.name || allExercises.find(e => e.id === activeAiExercise)?.name}
            </h2>
          </div>
          <AICoachPanel compact={false} exercise={activeAiExercise} />
        </div>
      ) : (
        <>
                  ...
                  <>
                    ...
                    {filteredAllExercises.slice(0, 10).map((exercise) => (
                      <motion.div 
                        ...
                        onClick={() => {
                          if (trackingMode === 'ai') {
                            setActiveAiExercise(exercise.id);
                            setIsSearchFocused(false);
                          } else {
